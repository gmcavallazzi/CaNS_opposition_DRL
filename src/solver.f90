...
module mod_solver
  ...
  use decomp_2d
  use mod_common_mpi, only: ipencil_axis, myid
  use mod_fft       , only: fft
  use mod_types
  use mod_param          , only: irk
  implicit none
  private
  public solver
...
#endif
  contains
  subroutine solver(n,ng,arrplan,normfft,lambdaxy,a,b,c,bc,c_or_f,p,pfix)
    implicit none
    ...
    if(bc(0,3)//bc(1,3) == 'PP') then
      ...
      if (present(pfix)) then
        call gaussel(       n_z(1),n_z(2),n_z(3)-q,0,a,b,c,pz,lambdaxy,pfix)
      else
  ...
  !
  subroutine gaussel(nx,ny,n,nh,a,b,c,p,lambdaxy,pfix)
    implicit none
    ...
    integer :: i,j
    real(rp), dimension(n) :: aa,cc
    integer :: k
    !
    ! solve tridiagonal system
    !
    if(present(pfix)) then
      k = n/2 !could be any number in the domain outside of the boundaries
      aa(:) = a(:)
      cc(:) = c(:)
      aa(k) = 0.
      cc(k) = 0.
    ...
    if(present(lambdaxy)) then
      ...
      do j=1,ny
        do i=1,nx
          ...
          if (present(pfix).and.(i+j==2)) then
            bb(k) = 1.
            p(i,j,k) = 0.
            call dgtsv_homebrewed(n,aa,bb,cc,p(i,j,1:n))
          else
