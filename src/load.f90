...
#undef _DECOMP_X_IO
  ...
  subroutine load_all(io,filename,comm,ng,nh,lo,hi,u,v,w,p,time,istep)
    ...
    case('r')
      ! Debug prints
      if (myid==0) then
        print*, 'Debug: About to open file: ', trim(filename)
      end if
      call MPI_COMM_SIZE(comm, size, ierr_loc)
      if (myid==0) print*, 'Debug: Communicator size: ', size
      call MPI_BARRIER(comm, ierr_loc)

      if (myid==0) print*, '*load_all: starting file opening*'
      call MPI_FILE_OPEN(comm,filename, &
                         MPI_MODE_RDONLY,MPI_INFO_NULL,fh,ierr_loc)
      
      if(ierr_loc /= MPI_SUCCESS) then
        call MPI_ERROR_STRING(ierr_loc, error_string, error_len, ierr)
        if(myid == 0) print*, 'Debug: MPI Error: ', trim(error_string(1:error_len))
        return
      end if

      if (myid==0) print*, '*load_all: file opened*'
      !
  ...
  subroutine io_field_hdf5(io,filename,varname,ng,nh,lo,hi,var,meta,x_g,y_g,z_g)
    ...
    case('r')
      ...
      call h5pset_fapl_mpio_f(plist_id,canscomm,MPI_INFO_NULL,ierr)
      ...
      call MPI_Bcast(meta,2,MPI_REAL_RP,0,canscomm,ierr)
    case('w')
