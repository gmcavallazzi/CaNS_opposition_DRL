...
def plot_z_lambda_spectrum(results, component, direction='x', save_path=None):
    ...
    if np.max(Z) > 0:
        ...
        peak_z = z_plus[peak_z_idx]
        peak_lambda = lambda_k[peak_lambda_idx]
        ...
        ax.set_title(r'$\kappa' + wavenumber_symbol + '\phi_{' + comp_label + '}$', 
                    fontsize=16)
...
def main():
    ...
    # Parameters
    re_tau = 1083.0  # Reynolds number based on friction velocity
    ...
    # Rank 0 combines all results and generates plots
    if rank == 0:
        final_results = {}
