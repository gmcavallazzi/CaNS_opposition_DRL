"""
Training script for MADDPG with point-based architecture.

This script trains multi-agent DRL for turbulent flow control with:
- Point-based observations: 4096 agents, each with scalar observations [u, w] or [u, w, prev_action]
- Point-based actions: 4096 agents, each with scalar action value
- Observation-similarity consistency loss for smooth spatial transitions
- Actor with MLP architecture (no convolutions, operates on individual points)
- Critic with circular CNN (full 64×64 domain is periodic)
- Action field reconstruction handled automatically by environment
"""
...
def create_field_visualization(u_field: np.ndarray, w_field: np.ndarray,
    ...
    Args:
        ...
        action_field: [64, 64] action field (already reconstructed by environment)
    ...
    # Action field (no need to reconstruct - already 64×64)
...
def create_action_change_visualization(action_t: np.ndarray, action_t_prev: np.ndarray,
    ...
    """
    Visualize temporal action changes to detect bang-bang oscillations.
    ...
    axes[2].set_title(f'Delta Actions (bang-bang indicator)\n{change_stats}', fontsize=10)
    axes[2].set_xlabel('j (spanwise)', fontsize=9)
    axes[2].set_ylabel('i (streamwise)', fontsize=9)
    cbar = plt.colorbar(im2, ax=axes[2], fraction=0.046, pad=0.04)
...
def train_maddpg_point(
    config: Dict[str, Any],
    checkpoint_dir: str = "./checkpoints_point",
    logs_dir: str = "./logs_point",
    device: Optional[str] = None,
    resume_checkpoint: Optional[str] = None,
    resume_buffer: Optional[str] = None,
):
    """
    Train MADDPG agents with point-based architecture for flow control.
    ...
    # Create point-based environment
    env = STWParallelEnvPoint(config)

    # Get agent list and observation/action spaces
    agents = env.possible_agents
    obs_shape = env.observation_spaces[agents[0]].shape  # (2,) or (3,) - scalar per agent
    act_shape = env.action_spaces[agents[0]].shape  # (1,) - scalar per agent
    num_agents = len(agents)  # 4096 agents

    # Auto-detect input channels from observation space
    input_channels = obs_shape[0]  # Number of features per point
    include_prev_action = config.get('observation', {}).get('include_prev_action', False)

    print(f"Environment created with {num_agents} point-based agents")
    print(f"Observation shape per agent: {obs_shape} (scalar features)")
    ...
    print(f"Action shape per agent: {act_shape} (scalar)")
    ...
    # Memory estimation for point-based system
    obs_size = np.prod(obs_shape)  # 2 or 3 scalars
    act_size = np.prod(act_shape)  # 1 scalar
    estimated_memory_gb = buffer_size * num_agents * (obs_size + act_size) * 4 / (1024**3)

    print(f"\nMemory usage estimation:")
    print(f"- Buffer size: {buffer_size:,}")
    print(f"- Observation size per agent: {obs_size} (scalar)")
    print(f"- Action size per agent: {act_size} (scalar)")
    ...
    # Create shared policy MADDPG trainer for point-based architecture
    ...
    # Initialize point-based replay buffer with correct observation channels
    replay_buffer = BatchedReplayBufferPoint(buffer_size, num_agents, agents, obs_channels=input_channels)
    ...
    run_name = f"maddpg_point_agents{num_agents}_{current_time}"
    ...
    episode_zero_losses = []
    episode_global_mean_losses = []  # Track global zero-mean loss
    episode_consistency_losses = []  # Track consistency loss
    episode_actions = []

    # Track gradient metrics per episode (for end-of-episode logging)
    ...
    # Initialize previous actions (all zeros for first step) - scalar per agent
    prev_actions = {agent: np.zeros(1, dtype=np.float32) for agent in agents}

    # Track action field for visualization (64×64 reconstructed by environment)
    prev_action_field = np.zeros((64, 64), dtype=np.float32)

    print("Starting training with point-based architecture...")
    ...
    try:
        while total_steps < max_steps:
            ...
            # Stack all scalar observations into a single tensor
            all_obs = np.stack([observations[agent] for agent in agents]).astype(np.float32)

            # Convert to tensor: [4096, 2] or [4096, 3]
            all_obs_tensor = torch.FloatTensor(all_obs).to(device)

            # Get scalar actions for all agents in a single forward pass
            with torch.no_grad():
                all_actions = maddpg.select_actions_batched(all_obs_tensor)  # [4096, 1]
            ...
            # Convert to dictionary (scalar per agent)
            actions = {agent: all_actions[i] for i, agent in enumerate(agents)}

            # Step environment with actions (environment reconstructs 64×64 field)
            next_observations, rewards, terminations, truncations, infos = env.step(actions)

            # Prepare batch data for replay buffer (stacked scalars)
            obs_batch = np.stack([observations[agent] for agent in agents])  # [4096, 2] or [4096, 3]
            next_obs_batch = np.stack([next_observations[agent] for agent in agents])  # [4096, 2] or [4096, 3]
            act_batch = np.stack([actions[agent] for agent in agents])  # [4096, 1]
            prev_act_batch = np.stack([prev_actions[agent] for agent in agents])  # [4096, 1]
            rew_batch = np.array([rewards[agent] for agent in agents])  # [4096]
            done_batch = np.array([terminations[agent] for agent in agents])  # [4096]
            ...
            if replay_buffer.size > learning_starts and total_steps % train_freq == 0:
                ...
                for _ in range(gradient_steps):
                    ...
                    # Batched update for shared policy (point-based)
                    ...
                    batch_global_mean_losses.append(loss_breakdown['global_mean_loss'])
                    batch_consistency_losses.append(loss_breakdown['consistency_loss'])
                ...
                if grad_config.get('enable', False) and total_steps % grad_config.get('log_frequency', 100) == 0:
                    ...
                    if total_steps % 500 == 0:
                        ...
                        sample_obs = all_obs_tensor[:256]  # Sample of 256 agents
                ...
                episode_global_mean_losses.extend(batch_global_mean_losses)
                episode_consistency_losses.extend(batch_consistency_losses)
            ...
            if any(terminations.values()) or any(truncations.values()):
                ...
                # Log gradient metrics averaged over episode
                ...
                if episode % 10 == 0:
                    try:
                        ...
                        # Action field is already 64×64 (reconstructed by environment)
                        action_field = env.action_field

                        # Create field visualization with episode info
                        field_image = create_field_visualization(u_field, w_field, action_field, episode)

                        # Log to TensorBoard (HWC format)
                        writer.add_image('Fields/velocity_and_actions', field_image, episode, dataformats='HWC')

                        # Also log action change visualization (bang-bang detector)
                        if episode > 0:  # Need at least 2 episodes for comparison
                            action_change_image = create_action_change_visualization(
                                action_field, prev_action_field, episode
                            )
                            writer.add_image('Fields/action_changes', action_change_image, episode, dataformats='HWC')

                        # Update previous action field for next visualization
                        prev_action_field = action_field.copy()
                ...
                episode_actor_lr = []
                episode_critic_lr = []
                # Note: prev_action_field persists across episodes for visualization
        ...
        - Architecture: Point-based (4096 agents)
        """
...
def main():
    parser = argparse.ArgumentParser(description='Train MADDPG with point-based architecture for STW control')
    ...
    parser.add_argument('--config', type=str, default='config_point.yaml',
    ...
    # Setup directories
    checkpoint_dir = "./checkpoints_point"
    logs_dir = "./logs_point"
    ...
    print("\n" + "="*60)
    print("STW MADDPG Training with Point-Based Architecture")
    print("="*60)
    ...
    # Train MADDPG agents with point-based architecture
    train_maddpg_point(
        config=config,
