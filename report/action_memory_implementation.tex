...
\begin{equation}
    \mathbf{a}_{\text{field}}^{\text{prev}} \leftarrow \mathbf{a}_{\text{field}}^{\text{curr}} - \overline{\mathbf{a}_{\text{field}}^{\text{curr}}}
\end{equation}

where the mean subtraction enforces zero-mean constraint (mass conservation).
...
\begin{algorithmic}[1]
\STATE Forward pass all agents: $\mathbf{A}^{\text{raw}} = \pi_\theta(\mathbf{S})$
...
actions_raw = actor(obs_batch)  # [batch, 64, 8, 8]

# Differentiable zero-mean correction
global_mean = actions_raw.mean()  # Scalar, differentiable
actions = actions_raw - global_mean  # [batch, 64, 8, 8]

# Actions are now zero-mean AND gradients flow through!
\end{lstlisting}

During inference (decentralized):
\begin{lstlisting}[language=Python]
# Same correction applied at execution
actions_raw = actor(obs)
...
\midrule
Post-processing + per-tile loss & \textcolor{red}{No} & \textcolor{red}{No} \\
Differentiable + per-tile loss & \textcolor{green}{Yes} & \textcolor{red}{No} \\
Post-processing + global loss & \textcolor{red}{No} & \textcolor{green}{Yes} \\
\textbf{Differentiable + global loss} & \textcolor{green}{Yes} & \textcolor{green}{Yes} \\
\bottomrule
\end{tabular}
...
\midrule
Temporal loss on replay & \textcolor{red}{No} & \textcolor{red}{Confused} & \textcolor{orange}{Partial} \\
External filtering & \textcolor{green}{Yes} & \textcolor{red}{Broken} & \textcolor{red}{No} \\
\textbf{Prev action in obs} & \textcolor{green}{Yes} & \textcolor{green}{Clean} & \textcolor{green}{Yes} \\
\bottomrule
\end{tabular}
