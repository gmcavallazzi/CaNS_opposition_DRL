...
module mod_sanity
  ...
  subroutine test_sanity_solver(ng,lo,hi,n,n_x_fft,n_y_fft,lo_z,hi_z,n_z,dli,dzc,dzf,dzci,dzfi,dzci_g,dzfi_g, &
    ...
    dti = dt**(-1)
    call bounduvw(cbcvel,n,bcvel,nb,is_bound,.false.,dl,dzc,dzf,u,v,w,0._rp,lo)
    ...
    call bounduvw(cbcvel,n,bcvel,nb,is_bound,.true.,dl,dzc,dzf,u,v,w,0._rp,lo)
    ...
    call fftend(arrplan)
    call bounduvw(cbcvel,n,bcvel,nb,is_bound,.false.,dl,dzc,dzf,u,v,w,0._rp,lo) ! actually we are only interested in boundary condition in u
    ...
    call fftend(arrplan)
    call bounduvw(cbcvel,n,bcvel,nb,is_bound,.false.,dl,dzc,dzf,u,v,w,0._rp,lo) ! actually we are only interested in boundary condition in v
    ...
    call fftend(arrplan)
    call bounduvw(cbcvel,n,bcvel,nb,is_bound,.false.,dl,dzc,dzf,u,v,w,0._rp,lo) ! actually we are only interested in boundary condition in w
