...
module mod_bound
  ...
  contains
  subroutine bounduvw(cbc,n,bc,nb,is_bound,is_correc,dl,dzc,dzf,u,v,w,time,lo,pfix,istep,ng,comm,datadir)
    !
    ...
    integer,  intent(in), dimension(3), optional :: ng
    ...
    ! Apply BCs from 2D slices if optional parameters are provided
    if ((present(istep)).and.(present(ng)).and.(present(comm)).and.(present(datadir))) then
      call apply_wave_bc(time, lo, dl, n, is_bound, w, istep, ipencil_axis, comm, ng, datadir)
    endif
