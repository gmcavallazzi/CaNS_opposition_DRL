...
program cans
  ...
  call bounduvw(cbcvel,n,bcvel,nb,is_bound,.false.,dl,dzc,dzf,u,v,w,time,lo)
  ...
  if(iout2d > 0.and.mod(istep,max(iout2d,1)) == 0) then
    include 'out2d.h90'
    call test_2d_slice_io(datadir, ipencil_axis, n, ng, w(1:n(1),1:n(2),1:n(3))) 
  end if
  ...
  do while(.not.is_done)
    ...
    do irk=1,3
...
#endif
      dpdl(:) = dpdl(:) + f(:)
      call bounduvw(cbcvel,n,bcvel,nb,is_bound,.false.,dl,dzc,dzf,u,v,w,time,lo,pfix,istep,ng,MPI_COMM_WORLD,datadir)
      ...
      call bounduvw(cbcvel,n,bcvel,nb,is_bound,.true.,dl,dzc,dzf,u,v,w,time,lo,pfix,istep,ng,MPI_COMM_WORLD,datadir)
    ...
    if(iout0d > 0.and.mod(istep,max(iout0d,1)) == 0) then
      ...
      if(any(is_forced(:)).or.any(abs(bforce(:)) > 0.)) then
        ...
        call out0d(trim(datadir)//'tau.out',1,[taux(0,3)])
      end if
    end if
    write(fldnum,'(i7.7)') istep
    if(iout1d > 0.and.mod(istep,max(iout1d,1)) == 0) then
